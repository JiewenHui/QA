...
#                 W_complex = tf.Variable(tf.random_uniform([self.vocab_size, 1], 0, 2*math.pi),name="W",trainable = self.trainable)
    ...
    def pooling_graph(self):
        with tf.name_scope('pooling'):
            ...
            self.represent_real = self.max_pooling(self.qa_real)
            print(self.represent_real)
            self.represent_real=tf.reshape(self.represent_real,[self.batch_size,-1])
            print(self.represent_real)
            self.represent_img = self.max_pooling(self.qa_imag)
            self.represent_img=tf.reshape(self.represent_img,[self.batch_size,-1])
            print(self.represent_img)
            w = tf.Variable(tf.zeros(shape = [1,self.num_filters],name = 'W'))
            # w = tf.Variable(tf.zeros(shape = [1,141120],name = 'W'))
            self.represent_real_W=tf.multiply((tf.multiply(self.represent_real,w)),self.represent_img)
            ...
            self.represent = tf.concat([self.represent_real,self.represent_img,self.represent_real_W,self.overlap],1)
